...
def fetchLinks(html, duplicate=True): # TODO fix duplicate removal
    ...
    if not duplicate:
        existingFiles = glob.glob("*.mp3")
        for track in music:
            if track[1] in existingFiles:
                music.remove(track)
                removed+=1
    print("{} duplicate tracks removed".format(removed))
    pprint(music)
    input()
    return music
...
def main():
    os.chdir("downloads")
    while True:
        artist, album, page_url, duplicate = getUserPrefs()
        ...
        if user is "y":
            break
    print()
    music = fetchLinks(requests.get(page_url).text, duplicate)
